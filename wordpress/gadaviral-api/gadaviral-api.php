...
function gadv_stream_embed_url($raw) {
	$raw = trim((string) $raw);
	if ($raw === '') return '';
	...
	if (strpos($host, 'youtube.com') !== false || strpos($host, 'youtu.be') !== false) {
		$v = '';
		if (!empty($query['v'])) $v = $query['v'];
		elseif (strpos($host, 'youtu.be') !== false) $v = ltrim($path, '/');
		elseif (preg_match('#^/(live|embed|shorts)/([A-Za-z0-9_\\-]+)#', $path, $m)) $v = $m[2];
		if ($v !== '') return 'https://www.youtube.com/embed/' . rawurlencode($v) . '?autoplay=1';
		return '';
	}
	// Facebook
	if (strpos($host, 'facebook.com') !== false || strpos($host, 'fb.watch') !== false) {
		return 'https://www.facebook.com/plugins/video.php?href=' . rawurlencode($raw) . '&show_text=false&autoplay=true';
	}
	// Twitch
	if (strpos($host, 'twitch.tv') !== false) {
		$channel = ltrim($path, '/');
		$channel = explode('/', $channel)[0];
		if ($channel !== '') return 'https://player.twitch.tv/?channel=' . rawurlencode($channel) . '&parent=' . ($_SERVER['HTTP_HOST'] ?? 'www.gadaviral.com');
		return '';
	}
	return '';
}
...
function gadv_posts_media_upload($request) {
	...
	foreach ($files as $f) {
		...
		if (!is_wp_error($attach_id)) {
			...
			$attached[] = ['id' => intval($attach_id), 'url' => gadv_media_url($attach_id)];
			$position++;
		}
	}
	$p = get_post($post_id);
...
function gadv_posts_create($request) {
	...
	if ($type === 'LIVE') {
		...
		$stream_url = gadv_stream_embed_url($raw_stream);
		if ($stream_url === '') return new WP_Error('invalid', 'A livestream URL (YouTube, Facebook or Twitch) is required for LIVE posts', ['status' => 400]);
	}
	$postarr = [
	...
	if ($type === 'LIVE') update_post_meta(intval($post_id), 'gadv_stream_url', $stream_url);
